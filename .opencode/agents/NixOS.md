...
permission:
  ...
  webfetch: allow
  websearch: allow
  # external_directory permissions are managed by opencode.json (gitignored, generated by devShell shellHook)
---
...
modules/base/home.nix     # system-level user: users.users, age identity, env vars (uses `username`)
...
build.sh                  # helper: nh os build
.envrc                    # direnv flake integration for the repo itself
flake.nix                 # top-level flake: inputs, outputs, devShell (defines `username`)
opencode.json             # workspace config — GITIGNORED, generated by devShell shellHook from username
```

`specialArgs` passes `self`, `inputs`, `username`, `userFullName`, `gitUserName`, `gitUserEmail` to all host configs — modules can reference the flake source tree, flake inputs, and user identity directly.
...
hosts/<name>/configuration.nix
  └─ ../../modules/base/default.nix   (agenix module, nix settings, networking, base packages)
       └─ ./home.nix                  (system user account, age identity, env vars)
...
- `flake.nix` — **single source of truth for all user-specific values**. The `let` block defines `username`, `userFullName`, `gitUserName`, `gitUserEmail` and passes them via `specialArgs`. Add new personalizations here, not in modules:
  ...
  ```
- `modules/base/default.nix` — agenix module import, nix flakes/GC/allowUnfree, locale, networking (NetworkManager via `lib.mkDefault`, nftables, no firewall), system packages (git, gh, devenv, nh, opencode, yazi, zip/unzip). Imports `./home.nix`.
- `modules/base/home.nix` — **system-level user infrastructure**. Owns `users.users.${username}`, `age.identityPaths`, and `environment.variables` (`NH_OS_FLAKE`, `HYPRLAND_CONFIG`). Uses `config.users.users.${username}.home` for fork-safe home directory paths.
...
- **Base/Client split**: `modules/base/home.nix` handles system-level user infrastructure (user account, age identity, env vars). `modules/client/home.nix` handles desktop user config (home-manager dotfiles, programs, theming). This split means `base` can manage a user account without needing the full desktop stack, and `client` builds on top of it.
- `NH_OS_FLAKE` and `HYPRLAND_CONFIG` are set in `modules/base/home.nix`, using `"${self}"` as the flake path.
- **`opencode.json` is generated**: The workspace config is gitignored and auto-generated by the devShell's `shellHook` using `username` from `flake.nix`. This means `opencode.json` is always fork-safe — it derives from the single source of truth. Run `nix develop` (or let direnv do it) to generate it. The agent's own `external_directory` permissions are in `opencode.json`, not duplicated in this file.
- **`auth.json` is a runtime file**: OpenCode's `~/.local/share/opencode/auth.json` is managed by OpenCode itself, NOT by Nix/agenix. It was previously managed as `age.secrets.opencode`, which created a destructive cycle (build.sh → encrypt → nh os switch → agenix overwrites live auth.json with stale data → immutable file). Now `auth.json` lives purely as a runtime file, and the agent has write access to `~/.local/share/opencode/` via `opencode.json`.
- **`build.sh`** runs `nh os build` — a simple build helper. No more secret re-encryption.
- **`age.identityPaths`** still exists in `modules/base/home.nix` for potential future age-managed secrets, but there are currently no `age.secrets` defined.
